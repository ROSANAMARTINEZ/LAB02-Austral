...
try:
    ...
    MISSING_REQUIRED.append('matplotlib')

try:
    from scipy.stats import pearsonr, spearmanr, kruskal, chi2_contingency
except ModuleNotFoundError:
    MISSING_REQUIRED.append('scipy')

try:
    import plotly.express as px
    import plotly.graph_objects as go
except ModuleNotFoundError:
    MISSING_REQUIRED.append('plotly')

try:
    from dash import Dash, html, dcc, dash_table
except ModuleNotFoundError:
    MISSING_REQUIRED.append('dash')

try:
    import dash_bootstrap_components as dbc
except ModuleNotFoundError:
    MISSING_REQUIRED.append('dash-bootstrap-components')
...
if MISSING_REQUIRED:
    ...
    raise SystemExit(
        ...
        f"{missing}. Instálalas con:\n"
        f"  python3 -m pip install -r '{req_path}'\n"
        "y en VS Code selecciona ese mismo intérprete."
    )

ROOT_DIR = Path(__file__).resolve().parent if '__file__' in globals() else Path.cwd()
BASE_CANDIDATES = [
    *([Path(os.getenv('PETFINDER_BASE'))] if os.getenv('PETFINDER_BASE') else []),
    ROOT_DIR / 'input' / 'petfinder-adoption-prediction',
    ROOT_DIR.parent / 'input' / 'petfinder-adoption-prediction',
    Path('/Users/fernandopaganini/Desktop/Laboratorio 2/UA_MDM_Labo2/input/petfinder-adoption-prediction'),
]
BASE = next((p for p in BASE_CANDIDATES if p.exists()), BASE_CANDIDATES[-1])

OUTPUT = Path(os.getenv('PETFINDER_OUTPUT', ROOT_DIR / 'output'))
...
def px_heatmap_corr(corr_m):
    ...
    fig.update_layout(
        **chart_layout('Matriz de correlación — variables numéricas', height=max(420, len(corr_m) * 30)),
...
def graph(fig, fname=None):
    if fname:
        try:
            ...
            warnings.warn(f'No se pudo exportar {fname}.png ({exc})')
...
def _safe_upper_ratio(text_value):
...
def load_main():
    ...
    # Flags y features antes de mapear / dropear columnas crudas
    df['has_name'] = df['Name'].fillna('').str.strip().ne('').astype(int)
    ...
    df['desc_len_words'] = df['Description'].fillna('').str.split().str.len().fillna(0).astype(int)
    df['desc_exclamations'] = df['Description'].fillna('').str.count('!')
    df['desc_question_marks'] = df['Description'].fillna('').str.count(r'\?')
    df['desc_upper_ratio'] = df['Description'].fillna('').apply(_safe_upper_ratio)
    df['desc_unique_word_ratio'] = (
        df['Description'].fillna('').apply(lambda s: len(set(re.findall(r'\w+', s.lower()))) / max(len(re.findall(r'\w+', s.lower())), 1))
    )
    df['has_second_breed'] = df['Breed2'].fillna(0).ne(0).astype(int)
    df['has_color2'] = df['Color2'].fillna(0).ne(0).astype(int)
    df['has_color3'] = df['Color3'].fillna(0).ne(0).astype(int)
    df['n_colors'] = df[['Color1', 'Color2', 'Color3']].fillna(0).ne(0).sum(axis=1)
    df['has_fee'] = df['Fee'].fillna(0).gt(0).astype(int)
    df['is_baby'] = df['Age'].fillna(999).le(3).astype(int)
    df['is_adult'] = df['Age'].fillna(0).gt(12).astype(int)
    df['photo_per_pet'] = df['PhotoAmt'] / df['Quantity'].replace(0, np.nan)
    df['media_richness'] = df['PhotoAmt'].fillna(0) + 2 * df['VideoAmt'].fillna(0)
    df['age_bin'] = df['Age'].apply(_bin_age)
    df['log_fee'] = np.log1p(df['Fee'].fillna(0))
    df['log_age'] = np.log1p(df['Age'].fillna(0))
    df['log_photoamt'] = np.log1p(df['PhotoAmt'].fillna(0))
    df['rescuer_pet_count'] = df.groupby('RescuerID')['PetID'].transform('count')
    df['rescuer_fee_mean'] = df.groupby('RescuerID')['Fee'].transform('mean')
    df['rescuer_photo_mean'] = df.groupby('RescuerID')['PhotoAmt'].transform('mean')

    df['Type']         = df['Type'].map({1: 'Perro', 2: 'Gato'})
    df['Gender']       = df['Gender'].map({1: 'Macho', 2: 'Hembra', 3: 'Mixto'})
    df['Breed1_raw']   = df['Breed1']
    df['Breed2_raw']   = df['Breed2']
    df['Breed1']       = df['Breed1'].map(bmap).fillna('Desconocido')
    df['Breed2']       = df['Breed2'].map(bmap).fillna('Desconocido')
    df['same_breed']   = (df['Breed1'] == df['Breed2']).astype(int)
    ...
    # Targets auxiliares solo para EDA
    df['fast_adoption'] = df['AdoptionSpeed'].isin([0, 1]).astype(int)
    df['adopted_30d'] = df['AdoptionSpeed'].isin([0, 1, 2]).astype(int)
    df['slow_adoption'] = df['AdoptionSpeed'].isin([3, 4]).astype(int)

    # Mantengo texto crudo fuera del dashboard para no inflarlo
    df = df.drop(columns=['Description', 'RescuerID'], errors='ignore')
    return df
...
df_main             = load_main()
print('Cargando metadata (~58 000 archivos)…')
...
df['has_metadata'] = df['avg_label_score'].notna().astype(int) if 'avg_label_score' in df.columns else 0
df['has_sentiment'] = df['doc_score'].notna().astype(int) if 'doc_score' in df.columns else 0
for col in ['n_labels_img', 'avg_label_score', 'max_pixelFraction', 'crop_confidence',
            'doc_score', 'doc_magnitude', 'n_sentences', 'sentence_score_range']:
...
def fast_rate_by_category(col_name, top=None):
    valid = df[[col_name, 'fast_adoption']].dropna()
    if top:
        keep = valid[col_name].value_counts().head(top).index
        valid = valid[valid[col_name].isin(keep)]
    out = (valid.groupby(col_name)['fast_adoption']
                .mean()
                .sort_values(ascending=False)
                .mul(100)
                .round(1))
    return out.index.tolist(), out.values


def adopted30_rate_by_category(col_name, top=None):
    valid = df[[col_name, 'adopted_30d']].dropna()
    if top:
        keep = valid[col_name].value_counts().head(top).index
        valid = valid[valid[col_name].isin(keep)]
    out = (valid.groupby(col_name)['adopted_30d']
                .mean()
                .sort_values(ascending=False)
                .mul(100)
                .round(1))
...
def fast_rate_by_bins(col_name, bins, labels=None):
    valid = df[[col_name, 'fast_adoption']].dropna().copy()
    valid['bin'] = pd.cut(valid[col_name], bins=bins, labels=labels, include_lowest=True)
    out = (valid.groupby('bin', observed=False)['fast_adoption']
                .mean()
                .mul(100)
                .round(1))
    return out.index.astype(str).tolist(), out.values
...
SIDEBAR = html.Div([
    ...
    html.Div([
        ...
        *[html.Div([
        ...
        for icon, label in [('📊', 'Distribución'), ('🔗', 'Asociación'), ('✅', 'Significación')]],
    ], style={'padding': '1.25rem 1.5rem',
               'borderBottom': '1px solid rgba(255,255,255,0.08)'}),
    ...
    html.Div([
        ...
        html.P('petfinder-adoption-prediction', style={'color': '#374151', 'fontSize': '0.65rem',
...
KPI_ROW = dbc.Row([
    ...
    dbc.Col(kpi_card('Con Imágenes',       f'{n_dogs + n_cats:,}', f'🐕 {n_dogs:,}  🐈 {n_cats:,}', '📸', C_PURPLE), md=3),
], className='g-3 mb-4')
...
INFO_PANEL = info_panel('Resumen del Dataset', [
    ...
    ('Con descripción',   f"{df['has_description'].mean() * 100:.1f}%"),
    ('Con metadata img.', f"{df['has_metadata'].mean() * 100:.1f}%"),
    ('Con sentimiento',   f"{df['has_sentiment'].mean() * 100:.1f}%"),
    ('Adopción rápida',   f"{df['fast_adoption'].mean() * 100:.1f}%"),
    ('≤ 30 días',         f"{df['adopted_30d'].mean() * 100:.1f}%"),
])
...
def build_tab1():
    ...
    children.append(dbc.Row([
        ...
        dbc.Col([
            ...
            card([
                sub_title('Missingno — Completitud'),
                *[html.Div()],
            ])
        ], md=5),
    ]))
    ...
    if msno is not None:
        ...
        msno_bar = mpl_to_b64(plt.gcf(), 'msno_bar')
        plt.close('all')

        children[-1].children[1].children[1].children[-1] = html.Img(
            src=msno_mat, style={'width': '100%'})
        children.append(card([
            sub_title('Missingno — Barras de completitud'),
            html.Img(src=msno_bar, style={'width': '100%'}),
        ]))
    else:
        children[-1].children[1].children[1].children[-1] = html.P(
            'Instala `missingno` para ver esta visualización.',
    ...
    children.append(card([
        graph(px_area_bar(adop_ord.index.tolist(), adop_ord.values.tolist(),
                           'Distribución de AdoptionSpeed', 'Velocidad de adopción', 'N° de mascotas',
                           color=C_BLUE, height=350), 'dist_adoption_speed')
    ]))

    fr_labels = ['Adopción rápida (0-1)', 'Adoptado ≤ 30d (0-2)', 'Adopción lenta (3-4)']
    fr_vals = [round(df['fast_adoption'].mean() * 100, 1),
               round(df['adopted_30d'].mean() * 100, 1),
               round(df['slow_adoption'].mean() * 100, 1)]
    children.append(card([
        graph(px_bar(fr_labels, fr_vals,
                     'Targets auxiliares para EDA', yt='% de mascotas', color=C_GREEN, height=300),
              'target_flags_overview')
    ]))

    children.append(section_title('Texto y señales derivadas'))
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            ['Con nombre', 'Sin nombre'],
            [int(df['has_name'].sum()), int((1 - df['has_name']).sum())],
            'Presencia de nombre', yt='Cantidad', color=C_BLUE))]), md=4),
        dbc.Col(card([graph(px_bar(
            ['Con descripción', 'Sin descripción'],
            [int(df['has_description'].sum()), int((1 - df['has_description']).sum())],
            'Presencia de descripción', yt='Cantidad', color=C_GREEN))]), md=4),
        dbc.Col(card([graph(px_bar(
            ['Con fee', 'Fee = 0'],
            [int(df['has_fee'].sum()), int((1 - df['has_fee']).sum())],
            'Tarifa de adopción', yt='Cantidad', color=C_ORANGE))]), md=4),
    ], className='g-3'))

    children.append(dbc.Row([
        dbc.Col(card([graph(px_box_chart(df['name_len'], 'Longitud del nombre', 'Caracteres', C_BLUE))]), md=3),
        dbc.Col(card([graph(px_box_chart(df['desc_len_chars'], 'Largo de descripción', 'Caracteres', C_GREEN))]), md=3),
        dbc.Col(card([graph(px_box_chart(df['desc_len_words'], 'Palabras en descripción', 'Palabras', C_ORANGE))]), md=3),
        dbc.Col(card([graph(px_box_chart(df['rescuer_pet_count'], 'Mascotas por rescatista', 'Cantidad', C_PURPLE))]), md=3),
    ], className='g-3'))

    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            top_n(df['age_bin']).index.tolist(), top_n(df['age_bin']).values.tolist(),
            'Edad agrupada', yt='Cantidad', color=C_INDIGO))]), md=6),
        dbc.Col(card([graph(px_box_chart(df['photo_per_pet'], 'Fotos por mascota', 'Ratio', C_TEAL))]), md=6),
    ], className='g-3'))

    children.append(section_title('Variables generales'))
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            top_n(df['Type']).index.tolist(), top_n(df['Type']).values.tolist(),
            'Tipo de animal', yt='Cantidad', color=C_BLUE))]), md=6),
        dbc.Col(card([graph(px_bar(
            top_n(df['Gender']).index.tolist(), top_n(df['Gender']).values.tolist(),
    ...
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            top_n(df['MaturitySize']).index.tolist(), top_n(df['MaturitySize']).values.tolist(),
            'Tamaño de madurez', yt='Cantidad', color=C_INDIGO))]), md=6),
        dbc.Col(card([graph(px_bar(
            top_n(df['FurLength']).index.tolist(), top_n(df['FurLength']).values.tolist(),
            'Longitud del pelaje', yt='Cantidad', color=C_AMBER))]), md=6),
    ], className='g-3'))

    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            top_n(df['Health']).index.tolist(), top_n(df['Health']).values.tolist(),
            'Estado de salud', yt='Cantidad', color=C_RED))]), md=5),
        dbc.Col(card([graph(px_bar(
            top_n(df['State']).index.tolist(), top_n(df['State']).values.tolist(),
            'Estado (Malasia)', yt='', color=C_BLUE, horizontal=True, height=400))]), md=7),
    ], className='g-3'))

    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            top_n(df['Breed1']).index.tolist(), top_n(df['Breed1']).values.tolist(),
            'Raza principal (Top 9)', yt='', color=C_GREEN, horizontal=True, height=380))]), md=6),
        dbc.Col(card([graph(px_bar(
            top_n(df['Breed2']).index.tolist(), top_n(df['Breed2']).values.tolist(),
            'Raza secundaria (Top 9)', yt='', color=C_PURPLE, horizontal=True, height=380))]), md=6),
    ], className='g-3'))
    ...
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            top_n(df[cn]).index.tolist(), top_n(df[cn]).values.tolist(),
            f'Color — {cn}', yt='Cantidad', color=PALETTE[i]))]), md=4)
        for i, cn in enumerate(['Negro', 'Marrón', 'Dorado'])
    ], className='g-3'))
    ...
    if HAS_META:
        ...
        if not labels_df.empty:
            ...
            children.append(dbc.Row([
                dbc.Col(card([
                    sub_title('Top 20 etiquetas más frecuentes'),
                    graph(px_bar(lbl_freq.index.tolist(), lbl_freq.values.tolist(),
                                  '', yt='N° apariciones', color=C_BLUE,
                                  horizontal=True, height=450), 'top20_labels')
                ]), md=6),
                dbc.Col(card([
                    sub_title('Score vs Topicality — Top 15'),
                    graph(px_grouped_bar(lbl_st.index.tolist(),
                                          ...
                                          height=450), 'score_vs_topicality')
                ]), md=6),
            ], className='g-3'))

            lbl_wc = labels_df.groupby('description')['score'].mean().to_dict()
            if lbl_wc and WordCloud is not None:
                ...
                children.append(card([
                    sub_title('Nube de palabras — Etiquetas (score promedio)'),
                    html.P('Instala `wordcloud` para habilitar esta visualización.',
                           style={'margin': 0, 'color': TEXT_MUTED}),
                ]))
    ...
    if HAS_SENT:
        ...
        children.append(dbc.Row([
            dbc.Col(card([graph(px_bar(
                top_n(df['sentiment_class']).index.tolist(),
                top_n(df['sentiment_class']).values.tolist(),
                'Clasificación de sentimiento', yt='Cantidad', color=C_TEAL))]), md=5),
            dbc.Col(card([
                sub_title('Score de sentimiento por tipo de animal'),
                graph(px.box(
                    df[['Type', 'doc_score']].dropna(),
                    x='Type', y='doc_score', color='Type',
                    color_discrete_sequence=PALETTE,
                    labels={'Type': 'Tipo de animal', 'doc_score': 'Score de sentimiento'},
                ).update_layout(**chart_layout('', height=320, showlegend=False))),
            ]), md=7),
        ], className='g-3'))
        ...
        if not ent_df.empty:
            ...
            if ent_wc and WordCloud is not None:
                ...
                children.append(card([
                    sub_title('Nube de palabras — Entidades (salience promedio)'),
                    html.P('Instala `wordcloud` para habilitar esta visualización.',
                           style={'margin': 0, 'color': TEXT_MUTED}),
                ]))
...
def build_tab2():
    ...
    children.append(card([
        sub_title('Tabla de correlación — Variables numéricas'),
        dtable(corr_m.round(3).reset_index().rename(columns={'index': 'Variable'})),
    ]))
    children.append(card([
        sub_title('Heatmap de correlación'),
    ...
    children.append(card([
        sub_title('Lectura orientada a Kaggle'),
        html.P('Además del promedio de AdoptionSpeed, esta pestaña mira tasas de adopción rápida (0-1) y adopción en ≤ 30 días (0-2), que suelen ser más interpretables para feature engineering.',
               style={'color': TEXT_MUTED, 'marginBottom': '0'})
    ]))

    children.append(section_title('Tasas de adopción útiles'))
    age_labels, age_fast = fast_rate_by_bins('Age', bins=[-0.1, 2, 6, 12, 24, 500], labels=['0-2', '3-6', '7-12', '13-24', '24+'])
    fee_labels, fee_fast = fast_rate_by_bins('Fee', bins=[-0.1, 0, 50, 150, 1000, 100000], labels=['0', '1-50', '51-150', '151-1000', '1000+'])
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(age_labels, age_fast, 'Adopción rápida por tramo de edad', yt='% adopción rápida', color=C_BLUE))]), md=6),
        dbc.Col(card([graph(px_bar(fee_labels, fee_fast, 'Adopción rápida por fee', yt='% adopción rápida', color=C_ORANGE))]), md=6),
    ], className='g-3'))

    top_text_cats = [
        ('has_description', 'Descripción presente', C_GREEN),
        ('age_bin', 'Edad agrupada', C_PURPLE),
        ('Type', 'Tipo de animal', C_TEAL),
        ('Sterilized', 'Esterilizado', C_RED),
    ]
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            *fast_rate_by_category(cn),
            f'Tasa de adopción rápida — {lbl}',
            xt=lbl, yt='% adopción rápida', color=col, height=320))]), md=6)
        for cn, lbl, col in top_text_cats[:2]
    ], className='g-3'))
    children.append(dbc.Row([
        dbc.Col(card([graph(px_bar(
            *adopted30_rate_by_category(cn),
            f'Tasa de adopción ≤ 30 días — {lbl}',
            xt=lbl, yt='% adopción ≤ 30d', color=col, height=320))]), md=6)
        for cn, lbl, col in top_text_cats[2:]
    ], className='g-3'))

    def assoc_bar(x_labels, y_vals, title, xt='', yt='', col=C_BLUE):
        return graph(px_bar(x_labels, y_vals.tolist(), title, xt=xt, yt=yt, color=col, height=320))
    ...
    cat_pairs = [('Type','Tipo de animal', C_BLUE, None), ('Gender','Género', C_GREEN, None),
                 ('MaturitySize','Tamaño madurez', C_PURPLE, None), ('FurLength','Pelaje', C_ORANGE, None),
                 ('Health','Salud', C_RED, None)]
    children.append(dbc.Row([
        dbc.Col(card([assoc_bar(*cat_vs_adoption(cn), f'AdoptionSpeed — {lbl}', lbl,
                                 'AdoptionSpeed prom.', col)]), md=4)
        for cn, lbl, col, _ in cat_pairs[:3]
    ], className='g-3'))
    children.append(dbc.Row([
        dbc.Col(card([assoc_bar(*cat_vs_adoption(cn), f'AdoptionSpeed — {lbl}', lbl,
                                 'AdoptionSpeed prom.', col)]), md=6)
        for cn, lbl, col, _ in cat_pairs[3:]
    ], className='g-3'))

    for cn, lbl, topk, col in [('State','Estado',10,C_TEAL),
                                 ('Breed1','Raza principal',9,C_INDIGO),
                                 ('Breed2','Raza secundaria',9,C_AMBER)]:
        xl, yv = cat_vs_adoption(cn, top=topk)
        children.append(card([assoc_bar(xl, yv, f'AdoptionSpeed — {lbl} (Top {topk})', lbl,
                                         'AdoptionSpeed prom.', col)]))
    ...
    if HAS_SENT:
        ...
        sent_pairs = [('doc_score','Score global',C_BLUE), ('doc_magnitude','Magnitud',C_GREEN),
                      ('n_sentences','N° de oraciones',C_ORANGE), ('sentence_score_range','Variación emoc.',C_PURPLE)]
        children.append(dbc.Row([
            dbc.Col(card([assoc_bar(*num_vs_adoption(cn), f'{lbl} por AdoptionSpeed',
                                     'Velocidad de adopción', lbl, col)]), md=6)
        ...
        if 'sentiment_class' in df.columns:
            ...
            children.append(card([
                sub_title('Cruce: Clasificación de sentimiento × AdoptionSpeed'),
                graph(px_multibar_stacked(cross, 'Distribución de AdoptionSpeed por sentimiento',
                                           xt='Velocidad de adopción',
                                           yt='% dentro de la clase'), 'cross_sentiment_adoption'),
            ]))
...
def build_tab3():
    ...
    children.append(dbc.Alert(
        f'{"✅  " + str(len(sig_pairs)) + " pares con |coeficiente| > 0.30 encontrados."if not sig_pairs.empty else "⚠️  Sin pares con |coeficiente| > 0.30. Se muestran los 10 más fuertes."}',
    ...
    for _, row_p in work_pairs.iterrows():
        ...
        if tipo == 'num-num':
            ...
            if len(valid) >= 10:
                try:
                    stat_v, p_v = spearmanr(valid[v1], valid[v2])
                except Exception:
                    pass
        elif tipo == 'cat-num':
...
app.title = 'PetFinder — Adoption Prediction EDA'
